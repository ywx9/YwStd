...
template<class _Ty> _NODISCARD constexpr int _Countl_zero_internal(const _Ty _Val) noexcept {
...
struct
  ...
  static constexpr unsigned char _AddCarry64(unsigned char _Carry, uint64_t _Left, uint64_t _Right,
  ...
  static constexpr unsigned char _SubBorrow64(unsigned char _Carry, uint64_t _Left, uint64_t _Right,
                                              uint64_t& _Result) noexcept {
    // _STL_INTERNAL_CHECK(_Carry < 2);
#if _STL_128_INTRINSICS
  ...
  template<size_t __m, size_t __n> static constexpr void
  _Knuth_4_3_1_M(const uint32_t (&__u)[__m], const uint32_t (&__v)[__n], uint32_t (&__w)[__n + __m]) noexcept {
  ...
  static constexpr void _Knuth_4_3_1_D(uint32_t* const __u, const size_t __u_size, const uint32_t* const __v,
    ...
    for (int __j = __m; __j >= 0; --__j) {
      ...
      while ((__qhat >> 32) != 0 || static_cast<uint32_t>(__qhat) * static_cast<uint64_t>(__v[__n - 2]) >
                                      ((__rhat << 32) | __u[__j + __n - 2])) {
        --__qhat;
        __rhat += __v[__n - 1];
        if ((__rhat >> 32) != 0) { break; }
      }
  ...
  _NODISCARD static constexpr uint64_t _UDiv128(uint64_t _High, uint64_t _Low, uint64_t _Div,
                                                uint64_t& _Remainder) noexcept {
  ...
  _TEMPLATE_CLASS_INTEGRAL(_Ty)
  _NODISCARD constexpr explicit operator _Ty() const noexcept { return static_cast<_Ty>(_Word[0]); }

  _NODISCARD constexpr explicit operator bool() const noexcept { return (_Word[0] | _Word[1]) != 0; }
  ...
  _NODISCARD friend constexpr bool operator<(const _Base128& _Left, const _Base128& _Right) noexcept {
    if (_Left._Word[1] < _Right._Word[1]) { return true; }

    if (_Left._Word[1] > _Right._Word[1]) { return false; }
  ...
  _NODISCARD static constexpr _Base128 _Divide(_Base128 _Num, _Base128 _Den) noexcept {
    ...
    if (_Den._Word[1] >= _Num._Word[1]) {
      if (_Den._Word[1] > _Num._Word[1]) { return 0; }
    ...
    __qhat._Word[0] = _UDiv128(_High_digit >= _Den._Word[1] ? _High_digit - _Den._Word[1] : _High_digit, _Num._Word[1],
                               _Den._Word[1], __rhat);
    ...
    for (;;) {
      if (__qhat._Word[1] > 0) {
        ...
        if (_Prod <= _Base128{_Num._Word[0], __rhat}) { break; }
        --__qhat._Word[0];
      }

      const auto _Sum = __rhat + _Den._Word[1];
      if (__rhat > _Sum) { break; }
      __rhat = _Sum;
    }
  ...
  _NODISCARD static constexpr _Base128 _Modulo(_Base128 _Num, _Base128 _Den) noexcept {
    // establish _Den < _Num and _Num._Word[1] > 0
    if (_Den._Word[1] >= _Num._Word[1]) {
      if (_Den._Word[1] > _Num._Word[1]) { return _Num; }
    ...
    uint64_t __rhat _ZERO_OR_NO_INIT;
    uint64_t __qhat = _UDiv128(_High_digit >= _Den._Word[1] ? _High_digit - _Den._Word[1] : _High_digit, _Num._Word[1],
                               _Den._Word[1], __rhat);

    for (;;) {
      if (__qhat_high > 0) {
        if (__qhat-- == 0) { --__qhat_high; }
      } else {
        _Base128 _Prod;
...
struct _Unsigned128 : _Base128 {
...
#if _HAS_CXX20
  _NODISCARD friend constexpr strong_ordering operator<=>(const _Unsigned128& _Left,
                                                          const _Unsigned128& _Right) noexcept {
    strong_ordering _Ord = _Left._Word[1] <=> _Right._Word[1];
    if (_Ord == strong_ordering::equal) { _Ord = _Left._Word[0] <=> _Right._Word[0]; }
    return _Ord;
  }
#else  // ^^^ _HAS_CXX20 / !_HAS_CXX20 vvv
  _NODISCARD friend constexpr bool operator<(const _Unsigned128& _Left, const _Unsigned128& _Right) noexcept {
    ...
    if (_Right._Word[1] < _Left._Word[1]) { return false; }
  ...
  _NODISCARD constexpr _Unsigned128 operator+() const noexcept { return *this; }

  _NODISCARD constexpr _Unsigned128 operator-() const noexcept { return _Unsigned128{} - *this; }

  _NODISCARD constexpr _Unsigned128 operator~() const noexcept { return _Unsigned128{~_Word[0], ~_Word[1]}; }
  ...
  _TEMPLATE_CLASS_INTEGRAL(_Ty)
  friend constexpr _Ty& operator%=(_Ty& _Left, const _Unsigned128& _Right) noexcept {
    if (_Right._Word[1] == 0) { _Left %= _Right._Word[0]; }
    return _Left;
  }
...
template<> class numeric_limits<_Unsigned128> : public _Num_int_base {
public:
  _NODISCARD static constexpr _Unsigned128(min)() noexcept { return 0; }

  _NODISCARD static constexpr _Unsigned128(max)() noexcept { return _Unsigned128{~0ull, ~0ull}; }

  _NODISCARD static constexpr _Unsigned128 lowest() noexcept { return (min)(); }

  _NODISCARD static constexpr _Unsigned128 epsilon() noexcept { return 0; }

  _NODISCARD static constexpr _Unsigned128 round_error() noexcept { return 0; }

  _NODISCARD static constexpr _Unsigned128 denorm_min() noexcept { return 0; }

  _NODISCARD static constexpr _Unsigned128 infinity() noexcept { return 0; }

  _NODISCARD static constexpr _Unsigned128 quiet_NaN() noexcept { return 0; }

  _NODISCARD static constexpr _Unsigned128 signaling_NaN() noexcept { return 0; }
...
struct _Signed128 : _Base128 {
...
#if _HAS_CXX20
  _NODISCARD friend constexpr strong_ordering operator<=>(const _Signed128& _Left, const _Signed128& _Right) noexcept {
  ...
  _NODISCARD friend constexpr bool operator<(const _Signed128& _Left, const _Signed128& _Right) noexcept {
    if (static_cast<int64_t>(_Left._Word[1]) < static_cast<int64_t>(_Right._Word[1])) { return true; }

    if (static_cast<int64_t>(_Right._Word[1]) < static_cast<int64_t>(_Left._Word[1])) { return false; }
  ...
  _NODISCARD constexpr _Signed128 operator+() const noexcept { return *this; }

  _NODISCARD constexpr _Signed128 operator-() const noexcept { return _Signed128{} - *this; }

  _NODISCARD constexpr _Signed128 operator~() const noexcept { return _Signed128{~_Word[0], ~_Word[1]}; }
...
template<> class numeric_limits<_Signed128> : public _Num_int_base {
public:
  _NODISCARD static constexpr _Signed128(min)() noexcept { return _Signed128{0ull, 1ull << 63}; }

  _NODISCARD static constexpr _Signed128(max)() noexcept { return _Signed128{~0ull, ~0ull >> 1}; }

  _NODISCARD static constexpr _Signed128 lowest() noexcept { return (min)(); }

  _NODISCARD static constexpr _Signed128 epsilon() noexcept { return 0; }

  _NODISCARD static constexpr _Signed128 round_error() noexcept { return 0; }

  _NODISCARD static constexpr _Signed128 denorm_min() noexcept { return 0; }

  _NODISCARD static constexpr _Signed128 infinity() noexcept { return 0; }

  _NODISCARD static constexpr _Signed128 quiet_NaN() noexcept { return 0; }

  _NODISCARD static constexpr _Signed128 signaling_NaN() noexcept { return 0; }
...
template<> struct common_type<_Signed128, _Unsigned128> {
  using type = _Unsigned128;
};
template<> struct common_type<_Unsigned128, _Signed128> {
  using type = _Unsigned128;
};

template<class _Ty> _NODISCARD constexpr _Ty _Min_limit() noexcept;

template<> _NODISCARD constexpr _Unsigned128 _Min_limit<_Unsigned128>() noexcept { return 0; }

template<> _NODISCARD constexpr _Signed128 _Min_limit<_Signed128>() noexcept { return _Signed128{0ull, 1ull << 63}; }

template<class _Ty> _NODISCARD constexpr _Ty _Max_limit() noexcept;

template<> _NODISCARD constexpr _Unsigned128 _Max_limit<_Unsigned128>() noexcept { return _Unsigned128{~0ull, ~0ull}; }

template<> _NODISCARD constexpr _Signed128 _Max_limit<_Signed128>() noexcept { return _Signed128{~0ull, ~0ull >> 1}; }
