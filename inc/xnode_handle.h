...
template<class _Iterator, class _NodeType> struct _Insert_return_type {
  _Iterator position;
  bool inserted;
  _NodeType node;
};
...
struct _Node_handle_map_base { // map-specific node handle behavior
  ...
  key_type& key() const noexcept /* strengthened */ { return _Datum().first; }
  mapped_type& mapped() const noexcept /* strengthened */ { return _Datum().second; }
...
template<class _Derived_type, class _ValueTy> struct _Node_handle_set_base { // set-specific node handle behavior
...
template<class _Node, class _Alloc, template<class...> class _Base, class... _Types> class _Node_handle
...
public:
  ...
  _Nodeptr _Getptr() const noexcept { return _Ptr; }
  ...
  _NODISCARD bool empty() const noexcept { return _Ptr == nullptr; }
  ...
  friend void swap(_Node_handle& _Left, _Node_handle& _Right) noexcept /* strengthened */ { _Left.swap(_Right); }
