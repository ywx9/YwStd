...
struct scheduler_ptr {
  ...
  /// </summary>
  explicit scheduler_ptr(::std::shared_ptr<scheduler_interface> _Scheduler)
    : _M_sharedScheduler(::std::move(_Scheduler)) {
  ...
  /// </summary>
  scheduler_interface* get() const { return _M_scheduler; }
...
public:
  ...
  /// </param>
  /**/
  explicit invalid_operation(_In_z_ const char* _Message) noexcept : exception(_Message) {}
...
public:
  ...
  /// </param>
  /**/
  explicit task_canceled(_In_z_ const char* _Message) noexcept : exception(_Message) {}
...
public:
  explicit _Interruption_exception(const char* _Message) noexcept : exception(_Message) {}
...
template<typename _T> _T atomic_compare_exchange(::std::atomic<_T>& _Target, _T _Exchange, _T _Comparand) {
  _T _Result = _Comparand;
  _Target.compare_exchange_strong(_Result, _Exchange);
  return _Result;
}

template<typename _T> _T atomic_exchange(::std::atomic<_T>& _Target, _T _Value) { return _Target.exchange(_Value); }

template<typename _T> _T atomic_increment(::std::atomic<_T>& _Target) { return _Target.fetch_add(1) + 1; }

template<typename _T> _T atomic_decrement(::std::atomic<_T>& _Target) { return _Target.fetch_sub(1) - 1; }

template<typename _T> _T atomic_add(::std::atomic<_T>& _Target, _T _Value) {
