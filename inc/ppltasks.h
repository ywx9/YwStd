...
#pragma warning(disable : 4793) // function compiled as native: Found an intrinsic not supported in managed code (caused
                                // by _ReturnAddress)
...
// In debug builds, default to 10 frames, unless this is overridden prior to #include'ing ppltasks.h.  In retail builds,
...
#if defined(__cplusplus_winrt)
template<typename _Type> struct _Unhat {
  typedef _Type _Value;
};

template<typename _Type> struct _Unhat<_Type ^> {
  typedef _Type _Value;
};
...
template<typename _Type, bool _IsValueTypeOrRefType = __is_valid_winrt_type(_Type)> struct _ValueTypeOrRefType {
  typedef _NonUserType _Value;
};

template<typename _Type> struct _ValueTypeOrRefType<_Type, true> {
  typedef _Type _Value;
};
...
template<typename _T1> _T1 _ProgressTypeSelector(Windows::Foundation::IAsyncActionWithProgress<_T1> ^);

template<typename _Type> struct _GetProgressType {
...
template<typename _T1, typename _T2> _TypeSelectorAsyncOperationWithProgress
  _AsyncOperationKindSelector(Windows::Foundation::IAsyncOperationWithProgress<_T1, _T2> ^);

template<typename _Ty>
_TypeSelectorAsyncActionWithProgress _AsyncOperationKindSelector(Windows::Foundation::IAsyncActionWithProgress<_Ty> ^);

template<typename _Type, bool _IsAsync = _IsIAsyncInfo<_Type>::_Value> struct _TaskTypeTraits {
...
template<typename _Type> struct _TaskTypeTraits<_Type, true> {
...
template<typename _Type> struct _ResultHolder<_Type ^> {
...
//
template<typename _Type> struct _ResultHolder<::std::vector<_Type ^>> {
  void Set(const ::std::vector<_Type ^>& _type) {
    _Result.reserve(_type.size());

    for (auto _PTask = _type.begin(); _PTask != _type.end(); ++_PTask) { _Result.emplace_back(*_PTask); }
  }
...
template<typename _Type> struct _ResultHolder<::std::pair<_Type ^, void*>> {
...
public:
  ...
  // A variable that remembers if this exception was ever rethrown into user code (and hence handled by the user).
  ...
  // this exception. If the call is to task_completion_event::set_exception, the set_exception method was the source of
  // the exception. DO NOT REMOVE THIS VARIABLE. It is extremely helpful for debugging.
...
public:
  ...
  virtual property Windows::Foundation::AsyncOperationCompletedHandler<_Result> ^ Completed {
    Windows::Foundation::AsyncOperationCompletedHandler<_Result> ^ get() { return _M_CompletedHandler; }
      ...
      void
      set(Windows::Foundation::AsyncOperationCompletedHandler<_Result> ^ _Value) {
...
/// </summary>
template<typename _Result, typename _Progress> ref struct _IAsyncOperationWithProgressToAsyncOperationConverter sealed
  ...
  internal : _IAsyncOperationWithProgressToAsyncOperationConverter(
    ...
    : _AsyncInfoImpl < Windows::Foundation::IAsyncOperationWithProgress<_Result, _Progress> ^
  , Windows::Foundation::AsyncOperationWithProgressCompletedHandler<_Result, _Progress>, _Result > (_Operation) {}
...
ref struct _IAsyncActionToAsyncOperationConverter sealed
  : _AsyncInfoImpl<Windows::Foundation::IAsyncAction ^, Windows::Foundation::AsyncActionCompletedHandler,
                   details::_Unit_type> {
...
public:
  virtual details::_Unit_type GetResults() override {
    // Invoke GetResults on the IAsyncAction to allow exceptions to be thrown to higher layers before returning a dummy
    // value.
...
/// </summary>
template<typename _Progress> ref struct _IAsyncActionWithProgressToAsyncOperationConverter sealed
  ...
  internal
    : _IAsyncActionWithProgressToAsyncOperationConverter(Windows::Foundation::IAsyncActionWithProgress<_Progress> ^
                                                         _Action)
    : _AsyncInfoImpl < Windows::Foundation::IAsyncActionWithProgress<_Progress> ^
  , Windows::Foundation::AsyncActionWithProgressCompletedHandler<_Progress>, details::_Unit_type > (_Action) {}
public:
  virtual details::_Unit_type GetResults() override {
...
public:
  ...
  /// </returns>
  /// <remarks>
  ///     The default context is used if you don't specify a continuation context when you call the <c>then</c> method.
  ///     In Windows applications for Windows 7 and below, as well as desktop applications on Windows 8 and higher, the
  ///     runtime determines where task continuations will execute. However, in a Windows Store app, the default
  ///     continuation context for a continuation on an apartment aware task is the apartment where <c>then</c> is
  ///     invoked. <para>An apartment aware task is a task that unwraps a Windows Runtime <c>IAsyncInfo</c> interface,
  ///     or a task that is descended from such a task. Therefore, if you schedule a continuation on an apartment aware
  ///     task in a Windows Runtime STA, the continuation will execute in that STA.</para> <para>A continuation on a
  ///     non-apartment aware task will execute in a context the Runtime chooses.</para>
  /// </remarks>
  /**/
  ...
  ///     It is very similar to what <c>task_continuation_context::use_current()</c> is doing except it is also
  ///     available without C++/Cx extension support. Please approach this method with caution as it is designed for
  ///     advanced users writing C++/Cx agnostic library code. <c>task_continuation_context::use_current()</c> will give
  ///     you more protection from mistakes. <para>This method is only available to Windows Store apps.</para>
  /// </remarks>
  /**/
  ...
  /// </returns>
  /// <remarks>
  ///     When this continuation context is used the continuation will execute in a context the runtime chooses even if
  ///     the antecedent task is apartment aware. <para><c>use_arbitrary</c> can be used to turn off the default
  ...
  /// </returns>
  /// <remarks>
  ///     This method will force the continuation task synchronously running on the context causing its antecedent
  ///     task's completion; if the antecedent task has completed when the continuation is attached, the continuation
  ///     will synchronously run on the context that attaches the continuation.
  /// </remarks>
  /**/
...
#endif
  // To ensure that the task_continuation_context ctor DLL export is preserved, conditional context behavior for the
  // ctor calls above is achieved with the tag dispatched ctor overload below, which forwards to the appropriate
  // exported ctor.
  struct _Tag {};
...
public:
  ...
  /// <summary>
  ///     Task option that specify a cancellation token and a continuation context. This is valid only for continuations
  ///     (then)
  /// </summary>
...
struct _TaskEventLogger {
  ...
  // It will log the cancel event but not canceled state. _LogTaskCompleted will log the terminal state, which includes
  // cancel state.
...
struct _Task_impl_base {
  ...
  task_status _Wait() {
    ...
    if (_IsNonBlockingThread()) {
      ...
      if (!_IsCompleted() && !_IsCanceled()) {
        ...
        // Task Continuations are 'scheduled' *inside* the chore that is executing on the ancestor's task group. If a
        // continuation needs to be marshalled to a different apartment instead of scheduling, we make a synchronous
        // cross-apartment COM call to execute the continuation. If it then happens to do something which waits on the
        // ancestor (say it calls .get(), which task based continuations are wont to do), waiting on the task group
        // results in waiting on the chore that is making this synchronous callback, which causes a deadlock. To avoid
        // this, we test the state of the ancestor's event, and we will NOT wait on it if it has finished execution
        // (which means now we are in the inline synchronous callback).
        _DoWait = false;
      }
    }
#endif
    ...
    if (_DoWait) {
      ...
      if (_M_fFromAsync) {
        ...
        // For tasks with unwrapped tasks, we should not change the state of current task, since the unwrapped task are
        // still running.
        ...
        // must be called from code that is executed within the task (throwing it from parallel work created by and
        // waited upon by the task is acceptable). We can safely assume that the task wrapper _PPLTaskHandle::operator()
        // has seen the exception and canceled the task. Swallow the exception here.
        ...
        // which is to be unwrapped and plumbed to the output of this task, we must not only wait on the lambda body, we
        // must wait on the **INNER** body. It is in theory possible that we could inline such if we plumb a series of
        // things through; however, this simply waits upon the completion signal.
  ...
  ///     ancestor or task_completion_event the task was registered with were canceled with an exception. A synchronous
  ///     cancel is one that assures the task could not be running on a different thread at the time the cancellation is
  ///     in progress. An asynchronous cancel is one where the thread performing the cancel has no control over the
  ///     thread that could be executing the task, that is the task could execute concurrently while the cancellation is
  ///     in progress.
  /// </param>
  ...
  void _RunContinuation(_ContinuationTaskHandleBase* _PTaskHandle) {
    ...
    if (_IsCanceled() && !_PTaskHandle->_M_isTaskBasedContinuation) {
      ...
      // This can only run when the ancestor has completed or it's a task based continuation that fires when a task is
      // canceled (with or without a user exception).
  ...
  void _ScheduleContinuationTask(_ContinuationTaskHandleBase* _PTaskHandle) {
    ...
    if (_PTaskHandle->_M_continuationContext._HasCapturedContext()) {
      // For those continuations need to be scheduled inside captured context, we will try to apply automatic inlining
      // to their inline modes, if they haven't been specified as _ForceInline yet. This change will encourage those
      // continuations to be executed inline so that reduce the cost of marshaling. For normal continuations we won't do
      // any change here, and their inline policies are completely decided by ._ThenImpl method.
      ...
      _ScheduleFuncWithAutoInline(
        [_PTaskHandle]() {
          // Note that we cannot directly capture "this" pointer, instead, we should use _TaskImplPtr, a shared_ptr to
          // the _Task_impl_base. Because "this" pointer will be invalid as soon as _PTaskHandle get deleted.
          // _PTaskHandle will be deleted after being scheduled.
          ...
          if (details::_ContextCallback::_CaptureCurrent() == _PTaskHandle->_M_continuationContext) {
            ...
            //
            // It's entirely possible that the attempt to marshal the call into a differing context will fail. In this
            // case, we need to handle the exception and mark the continuation as canceled with the appropriate
            // exception. There is one slight hitch to this:
            //
            // NOTE: COM's legacy behavior is to swallow SEH exceptions and marshal them back as HRESULTS. This will in
            // effect turn an SEH into a C++ exception that gets tagged on the task. One unfortunate result of this is
            // that various pieces of the task infrastructure will not be in a valid state after this in /EHsc (due to
            // the lack of destructors running, etc...).
            //
            _TRY_BEGIN
  ...
  ///     implementation if the task has completed or append it to a list of functions to execute when the task actually
  ///     does complete.
  /// </summary>
...
#if defined(__cplusplus_winrt)
  template<typename _ReturnType, typename> static void _AsyncInit(
    ...
    _AsyncOp->Completed = ref new Windows::Foundation::AsyncOperationCompletedHandler<_ReturnType>(
      [_OuterTask](Windows::Foundation::IAsyncOperation<typename details::_ValueTypeOrRefType<_ReturnType>::_Value> ^
                     _Operation,
        ...
        if (_Status == Windows::Foundation::AsyncStatus::Canceled) {
          ...
          _OuterTask->_CancelWithException(::std::make_exception_ptr(
        ...
        // Take away this shared pointers reference on the task instead of waiting for the delegate to be released. It
        // could be released on a different thread after a delay, and not releasing the reference here could cause the
        // tasks to hold on to resources longer than they should. As an example, without this reset, writing to a file
        // followed by reading from it using the Windows Runtime Async APIs causes a sharing violation. Using const_cast
        // is the workaround for failed mutable keywords
  ...
  static void _AsyncInit(const typename _Task_ptr<_ReturnType>::_Type& _OuterTask,
    ...
    // presence of an exception flowing out of the inner task _UnwrappedTask. This requires an exception handling
    // continuation off the inner task which does the appropriate funnelling to the outer one. We use _Then instead of
    // then to prevent the exception from being marked as observed by our internal continuation. This continuation must
    // be scheduled regardless of whether or not the _OuterTask task is canceled.
    //
  ...
  // Set to true either if the ancestor task had the flag set to true, or if the lambda that does the work of this task
  // returns an async operation or async action that is unwrapped by the runtime.
  bool _M_fFromAsync;
  ...
  // An exception thrown by the task body is captured in an exception holder and it is shared with all value based
  // continuations rooted at the task. The exception is 'observed' if the user invokes get()/wait() on any of the tasks
  // that are sharing this exception holder. If the exception is not observed by the time the internal object owned by
  // the shared pointer destructs, the process will fail fast.
...
#pragma warning(pop)
  ...
  virtual ~_Task_impl() {
    // We must invoke _DeregisterCancellation in the derived class destructor. Calling it in the base class destructor
    // could cause a partially initialized _Task_impl to be in the list of registrations for a cancellation token.
  ...
  virtual bool _CancelAndRunContinuations(bool _SynchronousCancel, bool _UserException, bool _PropagatedFromAncestor,
    ...
    enum { _Nothing, _RunContinuations, _Cancel } _Do = _Nothing;
    {
      ...
      if (_UserException) {
        ...
        // Completed is a non-cancellable state, and if this is an asynchronous cancel, we're unable to do better than
        // the last async cancel which is to say, cancellation is already initiated, so return early.
      ...
      if (_SynchronousCancel) {
        // Be aware that this set must be done BEFORE _M_Scheduled being set, or race will happen between this and
        // wait()
        ...
        // The _M_TaskState variable transitions to _Canceled when cancellation is completed (the task is not executing
        // user code anymore). In the case of a synchronous cancel, this can happen immediately, whereas with an
        // asynchronous cancel, the task has to move from _Started to _PendingCancel before it can move to _Canceled
    ...
    case _Cancel: {
      if (_M_InternalCancellation) {
        // We will only try to cancel async operation but not unwrapped tasks, since unwrapped tasks cannot be canceled
        // without its token.
  ...
  void _SetUnwrappedAsyncOp(
    ...
    bool _DoCancel = false;
    {
      ...
      if (_IsPendingCancel()) {
        ...
        _M_TaskState = _Started;
        _M_InternalCancellation = [=] { _AsyncOp->Cancel(); };
      }
    }
    if (_DoCancel) { _AsyncOp->Cancel(); }
  }
#endif /* defined (__cplusplus_winrt) */
...
public:
  ...
  /// </param>
  /// <returns>
  ///     The method returns <c>true</c> if it was successful in setting the event. It returns <c>false</c> if the event
  ///     is already set.
  /// </returns>
  /// <remarks>
  ///     In the presence of multiple or concurrent calls to <c>set</c>, only the first call will succeed and its result
  ///     (if any) will be stored in the task completion event. The remaining sets are ignored and the method will
  ///     return false. When you set a task completion event, all the tasks created from that event will immediately
  ///     complete, and its continuations, if any, will be scheduled. Task completion objects that have a <typeparamref
  ///     name="_ResultType"/> other than <c>void</c> will pass the value <paramref value="_Result"/> to their
  ///     continuations.
  /// </remarks>
  /**/
  bool set(
    _ResultType _Result) const // 'const' (even though it's not deep) allows to safely pass events by value into lambdas
  {
  ...
  {
    // It is important that _CAPTURE_CALLSTACK() evaluates to the instruction after the call instruction for
    // set_exception.
  ...
  bool
  set_exception(::std::exception_ptr _ExceptionPtr)
...
public:
  ...
  /// <summary>
  ///     Cancel the task_completion_event with the exception holder provided. Any task created using this event will be
  ///     canceled with the same exception.
  /// </summary>
...
template<typename _ReturnType, typename _Ty> auto _IsValidTaskCtor(_Ty _Param, int, int, int, ...)
  -> decltype(_Param->GetResults(), ::std::true_type());
#endif
...
#if defined(__cplusplus_winrt)
                "incorrect argument for task constructor; must be a callable object, an asynchronous operation or a "
...
template<typename _Ty> static auto _IsValidCreateAsync(_Ty _Param, int, int, int, int)
  -> decltype(_Param(), ::std::true_type());

// A parameter lambda taking an cancellation_token argument is valid
template<typename _Ty> static auto _IsValidCreateAsync(_Ty _Param, int, int, int, ...)
  -> decltype(_Param(cancellation_token::none()), ::std::true_type());

// A parameter lambda taking a progress report argument is valid
template<typename _Ty> static auto _IsValidCreateAsync(_Ty _Param, int, int, ...)
...
template<typename _Ty> static auto _IsValidCreateAsync(_Ty _Param, int, ...)
  -> decltype(_Param(details::_ProgressReporterCtorArgType(), cancellation_token::none()), ::std::true_type());

// All else is invalid
template<typename _Ty> static ::std::false_type _IsValidCreateAsync(_Ty _Param, ...);
#endif /* defined (__cplusplus_winrt) */
}
/// <summary>
...
// A helper class template that transforms an initial task lambda returns void into a lambda that returns a non-void
// type (details::_Unit_type is used to substitute for void). This is to minimize the special handling required for
// 'void'.
...
public:
  ...
  ///     <c>get</c>, <c>wait</c> or <c>then</c> will throw an <see cref="invalid_argument Class">invalid_argument</see>
  ///     exception when called on a default constructed task. <para>A task that is created from a
  ///     <c>task_completion_event</c> will complete (and have its continuations scheduled) when the task completion
  ///     event is set.</para> <para>The version of the constructor that takes a cancellation token creates a task that
  ///     can be canceled using the <c>cancellation_token_source</c> the token was obtained from. Tasks created without
  ///     a cancellation token are not cancelable.</para> <para>Tasks created from a
  ///     <c>Windows::Foundation::IAsyncInfo</c> interface or a lambda that returns an <c>IAsyncInfo</c> interface reach
  ///     their terminal state when the enclosed Windows Runtime asynchronous operation or action completes. Similarly,
  ///     tasks created from a lambda that returns a <c>task&lt;result_type&gt;</c> reach their terminal state when the
  ///     inner task reaches its terminal state, and not when the lambda returns.</para> <para><c>task</c> behaves like
  ///     a smart pointer and is safe to pass around by value. It can be accessed by multiple threads without the need
  ///     for locks.</para> <para>The constructor overloads that take a Windows::Foundation::IAsyncInfo interface or a
  ///     lambda returning such an interface, are only available to Windows Store apps.</para> <para>For more
  ///     information, see <see cref="Task Parallelism (Concurrency Runtime)"/>.</para>
  /// </remarks>
  /**/
  ...
  ///     <c>task_completion_event&lt;result_type&gt;</c> object, or a Windows::Foundation::IAsyncInfo if you are using
  ///     tasks in your Windows Store app. The lambda or function object should be a type equivalent to
  ///     <c>std::function&lt;X()&gt;</c>, where X can be a variable of type <c>result_type</c>,
  ...
  explicit task(_Ty _Param) {
    ...
    // Do not move the next line out of this function. It is important that _CAPTURE_CALLSTACK() evaluates to the call
    // site of the task constructor.
  ...
  /// </param>
  /// <remarks>
  ///     As <c>task</c> behaves like a smart pointer, after a copy assignment, this <c>task</c> objects represents the
  ///     same actual task as <paramref name="_Other"/> does.
  /// </remarks>
  /**/
  ...
  __declspec(noinline) // Ask for no inlining so that the _CAPTURE_CALLSTACK gives us the expected result
  auto
  then(const _Function& _Func) const -> typename details::_ContinuationTypeTraits<_Function, _ReturnType>::_TaskOfType {
    task_options _TaskOptions;
  ...
  __declspec(noinline) // Ask for no inlining so that the _CAPTURE_CALLSTACK gives us the expected result
  auto
  then(const _Function& _Func, task_options _TaskOptions) const ->
  ...
  ///     The cancellation token to associate with the continuation task. A continuation task that is created without a
  ///     cancellation token will inherit the token of its antecedent task.
  /// </param>
  ...
  __declspec(noinline) // Ask for no inlining so that the _CAPTURE_CALLSTACK gives us the expected result
  auto
  then(const _Function& _Func, cancellation_token _CancellationToken,
  ...
  /// </summary>
  /// <returns>
  ///     A <c>task_status</c> value which could be either <c>completed</c> or <c>canceled</c>. If the task encountered
  ///     an exception during execution, or an exception was propagated to it from an antecedent task, <c>wait</c> will
  ...
  /// <summary>
  ///     Returns the result this task produced. If the task is not in a terminal state, a call to <c>get</c> will wait
  ///     for the task to finish. This method does not return a value when called on a task with a <c>result_type</c> of
  ///     <c>void</c>.
  /// </summary>
  /// <returns>
  ...
  /// </returns>
  /// <remarks>
  ///     If the task is canceled, a call to <c>get</c> will throw a <see cref="task_canceled Class">task_canceled</see>
  ///     exception. If the task encountered an different exception or an exception was propagated to it from an
  ///     antecedent task, a call to <c>get</c> will throw that exception.
  /// </remarks>
  /**/
  ...
  /// <summary>
  ///     Determines whether the task unwraps a Windows Runtime <c>IAsyncInfo</c> interface or is descended from such a
  ///     task.
  /// </summary>
  /// <returns>
  ///     <c>true</c> if the task unwraps an <c>IAsyncInfo</c> interface or is descended from such a task, <c>false</c>
  ///     otherwise.
  /// </returns>
  /**/
...
private:
  ...
  template<typename _InternalReturnType, typename _Function, typename _TypeSelection> struct _InitialTaskHandle
    ...
    void _Init(details::_TypeSelectorAsyncOperationWithProgress) const {
      ...
      details::_Task_impl_base::_AsyncInit<_ReturnType, _InternalReturnType>(
        this->_M_pTask,
        ref new details::_IAsyncOperationWithProgressToAsyncOperationConverter<_InternalReturnType, _ProgressType>(
    ...
    void _Init(details::_TypeSelectorAsyncActionWithProgress) const {
      ...
      details::_Task_impl_base::_AsyncInit<_ReturnType, _InternalReturnType>(
        this->_M_pTask, ref new details::_IAsyncActionWithProgressToAsyncOperationConverter<_ProgressType>(
                          _LogWorkItemAndInvokeUserLambda(_M_function)));
    }
#endif /* defined (__cplusplus_winrt) */
  };
  ...
  struct _ContinuationTaskHandle
    ...
    void _Continue(::std::false_type, details::_TypeSelectorAsyncAction) const {
      ...
      details::_Task_impl_base::_AsyncInit<_NormalizedContinuationReturnType, _ContinuationReturnType>(
        this->_M_pTask, ref new details::_IAsyncActionToAsyncOperationConverter(_LogWorkItemAndInvokeUserLambda(
                          _Continuation_func_transformer<_InternalReturnType, _FuncOutputType>::_Perform(_M_function),
                          _M_ancestorTaskImpl->_GetResult())));
    }
    ...
    //
    // This is a straight task continuation which returns an async operation with progress which will be unwrapped for
    // continuation
    //
    ...
    void _Continue(::std::true_type, details::_TypeSelectorAsyncAction) const {
      ...
      _ResultTask._SetImpl(::std::move(_M_ancestorTaskImpl));
      details::_Task_impl_base::_AsyncInit<_NormalizedContinuationReturnType, _ContinuationReturnType>(
        this->_M_pTask, ref new details::_IAsyncActionToAsyncOperationConverter(
  ...
  void _TaskInitNoFunctor(Windows::Foundation::IAsyncOperationWithProgress<
                            typename details::_ValueTypeOrRefType<_ReturnType>::_Value, _Progress> ^
                          _AsyncOp) {
  ...
  auto _ThenImpl(const _Function& _Func, details::_ThenImplOptions& _Options) const ->
    ...
    // antecedent's token UNLESS this is a an exception handling continuation. In that case, we break the chain with a
    // _None. That continuation is never canceled unless the user explicitly passes the same token.
    //
...
public:
  ...
  __declspec(noinline) // Ask for no inlining so that the _CAPTURE_CALLSTACK gives us the expected result
  auto
  then(const _Function& _Func, task_options _TaskOptions = task_options()) const ->
  ...
  /// <summary>
  ///     An internal version of then that takes additional flags and executes the continuation inline. Used for runtime
  ///     internal continuations only.
  /// </summary>
  template<typename _Function> auto _Then(const _Function& _Func, details::_CancellationTokenState* _PTokenState,
                                          details::_TaskInliningMode_t _InliningMode = details::_ForceInline) const ->
    typename details::_ContinuationTypeTraits<_Function, void>::_TaskOfType {
...
template<typename _Ty> _Ty _GetUnwrappedType(Windows::Foundation::IAsyncOperation<_Ty> ^);
...
template<typename _Progress> void _GetUnwrappedType(Windows::Foundation::IAsyncActionWithProgress<_Progress> ^);
#endif /* defined (__cplusplus_winrt) */
...
#if defined(__cplusplus_winrt)
namespace details {
template<typename _Ty> task<_Ty> _To_task_helper(Windows::Foundation::IAsyncOperation<_Ty> ^ _Op) {
  return task<_Ty>(_Op);
}
...
inline task<void> _To_task_helper(Windows::Foundation::IAsyncAction ^ _Op) { return task<void>(_Op); }
...
template<typename _ProgressType> class _ProgressDispatcherBase {
public:
...
template<typename _ProgressType, typename _ClassPtrType> class _ProgressDispatcher
...
/**/
template<typename _ProgressType> class progress_reporter {
...
public:
  ...
  template<typename _ClassPtrType> static progress_reporter _CreateReporter(_ClassPtrType _Ptr) {
...
//
enum _AsyncResultType { SingleResult = 0x0001, MultipleResults = 0x0002 };
...
// void arg:
template<typename _Class, typename _ReturnType> void _Arg1ClassHelperThunk(_ReturnType (_Class::*)() const);

// void arg:
template<typename _Class, typename _ReturnType> void _Arg2ClassHelperThunk(_ReturnType (_Class::*)() const);
...
template<typename _ReturnType, typename _Arg1> _Arg1 _Arg1PFNHelperThunk(_ReturnType(__cdecl*)(_Arg1));

template<typename _ReturnType, typename _Arg1> void _Arg2PFNHelperThunk(_ReturnType(__cdecl*)(_Arg1));

template<typename _ReturnType, typename _Arg1> _ReturnType _ReturnTypePFNHelperThunk(_ReturnType(__cdecl*)(_Arg1));

template<typename _ReturnType, typename _Arg1> _OneArgumentFunctor _ArgumentCountHelper(_ReturnType(__cdecl*)(_Arg1));

template<typename _ReturnType, typename _Arg1> _Arg1 _Arg1PFNHelperThunk(_ReturnType(__stdcall*)(_Arg1));

template<typename _ReturnType, typename _Arg1> void _Arg2PFNHelperThunk(_ReturnType(__stdcall*)(_Arg1));

template<typename _ReturnType, typename _Arg1> _ReturnType _ReturnTypePFNHelperThunk(_ReturnType(__stdcall*)(_Arg1));

template<typename _ReturnType, typename _Arg1> _OneArgumentFunctor _ArgumentCountHelper(_ReturnType(__stdcall*)(_Arg1));

template<typename _ReturnType, typename _Arg1> _Arg1 _Arg1PFNHelperThunk(_ReturnType(__fastcall*)(_Arg1));

template<typename _ReturnType, typename _Arg1> void _Arg2PFNHelperThunk(_ReturnType(__fastcall*)(_Arg1));

template<typename _ReturnType, typename _Arg1> _ReturnType _ReturnTypePFNHelperThunk(_ReturnType(__fastcall*)(_Arg1));
...
template<typename _ReturnType> void _Arg1PFNHelperThunk(_ReturnType(__cdecl*)());

template<typename _ReturnType> void _Arg2PFNHelperThunk(_ReturnType(__cdecl*)());

template<typename _ReturnType> _ReturnType _ReturnTypePFNHelperThunk(_ReturnType(__cdecl*)());

template<typename _ReturnType> _ZeroArgumentFunctor _ArgumentCountHelper(_ReturnType(__cdecl*)());

template<typename _ReturnType> void _Arg1PFNHelperThunk(_ReturnType(__stdcall*)());

template<typename _ReturnType> void _Arg2PFNHelperThunk(_ReturnType(__stdcall*)());

template<typename _ReturnType> _ReturnType _ReturnTypePFNHelperThunk(_ReturnType(__stdcall*)());

template<typename _ReturnType> _ZeroArgumentFunctor _ArgumentCountHelper(_ReturnType(__stdcall*)());

template<typename _ReturnType> void _Arg1PFNHelperThunk(_ReturnType(__fastcall*)());

template<typename _ReturnType> void _Arg2PFNHelperThunk(_ReturnType(__fastcall*)());

template<typename _ReturnType> _ReturnType _ReturnTypePFNHelperThunk(_ReturnType(__fastcall*)());

template<typename _ReturnType> _ZeroArgumentFunctor _ArgumentCountHelper(_ReturnType(__fastcall*)());

template<typename _Ty> struct _FunctorArguments {
  static const size_t _Count = 0;
};

template<> struct _FunctorArguments<_OneArgumentFunctor> {
  static const size_t _Count = 1;
};

template<> struct _FunctorArguments<_TwoArgumentFunctor> {
  static const size_t _Count = 2;
};

template<typename _Ty> struct _FunctorTypeTraits {
...
template<typename _Ty> struct _FunctorTypeTraits<_Ty*> {
...
template<typename _Ty> struct _ProgressTypeTraits {
  static const bool _TakesProgress = false;
  typedef void _ProgressType;
};

template<typename _Ty> struct _ProgressTypeTraits<progress_reporter<_Ty>> {
  static const bool _TakesProgress = true;
  typedef _Ty _ProgressType;
};

template<typename _Ty, size_t _Count = _FunctorTypeTraits<_Ty>::_ArgumentCount> struct _CAFunctorOptions {
  static const bool _TakesProgress = false;
  static const bool _TakesToken = false;
  typedef void _ProgressType;
};

template<typename _Ty> struct _CAFunctorOptions<_Ty, 1> {
private:
...
template<typename _Ty> struct _CAFunctorOptions<_Ty, 2> {
private:
  typedef typename _FunctorTypeTraits<_Ty>::_Argument1Type _Argument1Type;
...
//
template<typename _AsyncSelector, typename _ReturnType> struct _SelectorTaskGenerator {
  template<typename _Function> static task<_ReturnType>
  _GenerateTask_0(const _Function& _Func, cancellation_token_source _Cts, const _TaskCreationCallstack& _callstack) {
  ...
  template<typename _Function> static task<_ReturnType>
  _GenerateTask_1C(const _Function& _Func, cancellation_token_source _Cts, const _TaskCreationCallstack& _callstack) {
    task_options _taskOptinos(_Cts.get_token());
    details::_get_internal_task_options(_taskOptinos)._set_creation_callstack(_callstack);
  ...
  static task<_ReturnType> _GenerateTask_1P(const _Function& _Func, const _ProgressObject& _Progress,
                                            cancellation_token_source _Cts, const _TaskCreationCallstack& _callstack) {
    task_options _taskOptinos(_Cts.get_token());
    details::_get_internal_task_options(_taskOptinos)._set_creation_callstack(_callstack);
  ...
  template<typename _Function, typename _ProgressObject>
  static task<_ReturnType> _GenerateTask_2PC(const _Function& _Func, const _ProgressObject& _Progress,
                                             cancellation_token_source _Cts, const _TaskCreationCallstack& _callstack) {
    task_options _taskOptinos(_Cts.get_token());
    details::_get_internal_task_options(_taskOptinos)._set_creation_callstack(_callstack);
...
template<typename _AsyncSelector> struct _SelectorTaskGenerator<_AsyncSelector, void> {
  template<typename _Function> static task<void> _GenerateTask_0(const _Function& _Func, cancellation_token_source _Cts,
  ...
  template<typename _Function> static task<void>
  _GenerateTask_1C(const _Function& _Func, cancellation_token_source _Cts, const _TaskCreationCallstack& _callstack) {
    task_options _taskOptinos(_Cts.get_token());
    details::_get_internal_task_options(_taskOptinos)._set_creation_callstack(_callstack);
  ...
  template<typename _Function, typename _ProgressObject>
  static task<void> _GenerateTask_1P(const _Function& _Func, const _ProgressObject& _Progress,
                                     cancellation_token_source _Cts, const _TaskCreationCallstack& _callstack) {
    task_options _taskOptinos(_Cts.get_token());
    details::_get_internal_task_options(_taskOptinos)._set_creation_callstack(_callstack);
  ...
  template<typename _Function, typename _ProgressObject>
  static task<void> _GenerateTask_2PC(const _Function& _Func, const _ProgressObject& _Progress,
                                      cancellation_token_source _Cts, const _TaskCreationCallstack& _callstack) {
    task_options _taskOptinos(_Cts.get_token());
    details::_get_internal_task_options(_taskOptinos)._set_creation_callstack(_callstack);
...
//
template<typename _ReturnType> struct _SelectorTaskGenerator<_TypeSelectorNoAsync, _ReturnType> {
...
template<> struct _SelectorTaskGenerator<_TypeSelectorNoAsync, void> {
  template<typename _Function> static task<void> _GenerateTask_0(const _Function& _Func, cancellation_token_source _Cts,
                                                                 const _TaskCreationCallstack& _callstack) {
    task_options _taskOptinos(_Cts.get_token());
    details::_get_internal_task_options(_taskOptinos)._set_creation_callstack(_callstack);
...
//
template<typename _ReturnType> struct _SelectorTaskGenerator<_TypeSelectorAsyncTask, _ReturnType> {
  template<typename _Function> static task<_ReturnType>
  _GenerateTask_0(const _Function& _Func, cancellation_token_source _Cts, const _TaskCreationCallstack& _callstack) {
    return _Func();
  }
...
template<> struct _SelectorTaskGenerator<_TypeSelectorAsyncTask, void> {
  template<typename _Function> static task<void> _GenerateTask_0(const _Function& _Func, cancellation_token_source _Cts,
                                                                 const _TaskCreationCallstack& _callstack) {
    return _Func();
  }
...
template<typename _Generator, bool _TakesToken, bool TakesProgress> struct _TaskGenerator {};

template<typename _Generator> struct _TaskGenerator<_Generator, false, false> {
...
template<typename _Generator> struct _TaskGenerator<_Generator, true, false> {
  template<typename _Function, typename _ClassPtr, typename _ProgressType>
  static auto _GenerateTask(const _Function& _Func, _ClassPtr _Ptr, cancellation_token_source _Cts,
                            const _TaskCreationCallstack& _callstack)
    -> decltype(_Generator::_GenerateTask_0(_Func, _Cts, _callstack)) {
    ...
    return _Generator::_GenerateTask_1C(::std::function<_ReturnType __cdecl(cancellation_token)>(_Func), _Cts,
                                        _callstack);
#else
...
template<typename _Generator> struct _TaskGenerator<_Generator, false, true> {
  template<typename _Function, typename _ClassPtr, typename _ProgressType>
  static auto _GenerateTask(const _Function& _Func, _ClassPtr _Ptr, cancellation_token_source _Cts,
                            const _TaskCreationCallstack& _callstack)
    -> decltype(_Generator::_GenerateTask_0(_Func, _Cts, _callstack)) {
    ...
    return _Generator::_GenerateTask_1P(
      ::std::function<_ReturnType __cdecl(const progress_reporter<_ProgressType>&)>(_Func),
      progress_reporter<_ProgressType>::_CreateReporter(_Ptr), _Cts, _callstack);
#else
...
template<typename _Generator> struct _TaskGenerator<_Generator, true, true> {
  template<typename _Function, typename _ClassPtr, typename _ProgressType>
  static auto _GenerateTask(const _Function& _Func, _ClassPtr _Ptr, cancellation_token_source _Cts,
                            const _TaskCreationCallstack& _callstack)
    -> decltype(_Generator::_GenerateTask_0(_Func, _Cts, _callstack)) {
#ifndef _PPLTASKS_NO_STDFUNC
    typedef decltype(_Func(details::declval<const progress_reporter<_ProgressType>&>(),
                           details::declval<cancellation_token>())) _ReturnType;
    return _Generator::_GenerateTask_2PC(
      ::std::function<_ReturnType __cdecl(const progress_reporter<_ProgressType>&, cancellation_token)>(_Func),
      progress_reporter<_ProgressType>::_CreateReporter(_Ptr), _Cts, _callstack);
#else
...
struct _AsyncAttributes<_ProgressTypeParam, _ReturnTypeParam, _TaskTraits, _TakesToken, true> {
  ...
  typedef Windows::Foundation::AsyncOperationWithProgressCompletedHandler<_ReturnType, _ProgressType>
...
template<typename _Function> struct _AsyncLambdaTypeTraits {
  ...
  typedef _AsyncAttributes<_ProgressType, typename _TaskTraits::_TaskRetType, _TaskTraits, _TakesToken, _TakesProgress>
    _AsyncAttributes;
};
...
struct _AsyncAttributesTaskGenerator {
  ...
  static task<typename _AsyncAttributesT::_ReturnType> _Generate_Task(const _Function& _Func, _ClassPtr _Ptr,
                                                                      cancellation_token_source _Cts,
                                                                      const _TaskCreationCallstack& _callstack) {
    return _AsyncAttributesT::_TaskGenerator::_GenerateTask<_Function, _ClassPtr, _AsyncAttributesT::_ProgressType>(
      _Func, _Ptr, _Cts, _callstack);
  }
};
...
// operations)
//
template<typename _Attributes, _AsyncResultType _ResultType = SingleResult> ref class _AsyncInfoBase abstract
  : _Attributes::_AsyncBaseType {
  internal :
...
public:
  virtual typename _Attributes::_ReturnType GetResults() { throw ::Platform::Exception::CreateException(E_UNEXPECTED); }
  ...
  virtual property Windows::Foundation::AsyncStatus Status {
    Windows::Foundation::AsyncStatus get() {
      ...
      //
      // Map our internal cancel pending to cancelled. This way "pending cancelled" looks to the outside as "cancelled"
      // but can still transition to "completed" if the operation completes without acknowledging the cancellation
      // request
      //
      switch (_Current) {
  ...
  virtual property typename _Attributes::_ProgressDelegateType ^
    Progress {
      typename typename _Attributes::_ProgressDelegateType ^ get() { return _GetOnProgress(); }
        ...
        void
        set(typename _Attributes::_ProgressDelegateType ^ _ProgressHandler) {
    ...
    virtual void
    Cancel() {
    if (_TransitionToState(_AsyncCancelPending)) { _OnCancel(); }
  }
...
virtual property typename _Attributes::_CompletionDelegateType ^
  Completed {
    typename _Attributes::_CompletionDelegateType ^
      ...
      void
      set(typename _Attributes::_CompletionDelegateType ^ _CompleteHandler) {
      _CheckValidStateForDelegateCall();
      ...
      if (InterlockedIncrement(&_M_CompleteDelegateAssigned) == 1) {
        ...
        MemoryBarrier();
        if (_IsTerminalState()) { _FireCompletion(); }
      } else {
      ...
      _M_completeDelegateContext._CallInContext(
        [=] {
          try {
            ...
            _M_completeDelegate = nullptr;
            if (!_IsHRCOMDisconnected(_ex->HResult)) { throw; }
          }
        },
  ...
  inline void _CheckValidStateForDelegateCall() {
    if (_M_currentStatus == _AsyncClosed) { throw ::Platform::Exception::CreateException(E_ILLEGAL_METHOD_CALL); }
  }
...
#pragma warning(pop)
    {
      if (_Current != _AsyncCompleted) { throw ::Platform::Exception::CreateException(E_ILLEGAL_METHOD_CALL); }
    }
...
private:
  ...
  inline bool _TransitionToState(const _AsyncStatusInternal _NewState) {
    ...
    case _AsyncStatusInternal::_AsyncStarted:
      if (_Current != _AsyncCreated) { return false; }
      break;
    case _AsyncStatusInternal::_AsyncCompleted:
      if (_Current != _AsyncStarted && _Current != _AsyncCancelPending) { return false; }
      break;
    case _AsyncStatusInternal::_AsyncCancelPending:
      if (_Current != _AsyncStarted) { return false; }
      break;
    ...
    case _AsyncStatusInternal::_AsyncClosed:
      if (!_IsTerminalState(_Current)) { return false; }
      break;
  ...
  inline bool _IsTerminalState(_AsyncStatusInternal status) {
    return (status == _AsyncError || status == _AsyncCanceled || status == _AsyncCompleted || status == _AsyncClosed);
  }
...
ref class __declspec(no_empty_identity_interface) _AsyncProgressBase abstract
  : _AsyncInfoBase<_Attributes, _ResultType> {};

template<typename _Attributes, _AsyncResultType _ResultType>
ref class __declspec(no_empty_identity_interface) _AsyncProgressBase<_Attributes, true, _ResultType> abstract
  : _AsyncInfoBase<_Attributes, _ResultType> {
  internal :
...
//
template<typename _Function> ref class _AsyncTaskGeneratorThunk sealed
...
protected:
  ...
  virtual void _OnStart() override {
    //
    // Call the appropriate task generator to actually produce a task of the expected type. This might adapt the user
    // lambda for progress reports, wrap the return result in a task, or allow for direct return of a task depending on
    // the form of the lambda.
    //
    _M_task = _AsyncAttributesTaskGenerator::_Generate_Task < _Function, _AsyncTaskGeneratorThunk<_Function> ^,
    _Attributes > (_M_func, this, _M_cts, _M_creationCallstack);
    _M_task.then([=](task<typename _Attributes::_ReturnType> _Antecedent) {
      try {
        _Antecedent.get();
      } catch (task_canceled&) { this->_TryTransitionToCancelled(); } catch (::Platform::Exception ^ _Ex) {
        this->_TryTransitionToError(_Ex->HResult);
      } catch (...) { this->_TryTransitionToError(E_FAIL); }
...
/**/
template<typename _Function> __declspec(noinline) details::_AsyncTaskGeneratorThunk<_Function> ^
  ...
  template<typename _ReturnType> task<::std::vector<_ReturnType>> _WhenAllVectorAndValue(
