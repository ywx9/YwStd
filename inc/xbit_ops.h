...
_NODISCARD inline unsigned long _Floor_of_log_2(size_t _Value) noexcept { // returns floor(log_2(_Value))
  ...
  while ((size_t{1} << _Result) > _Value) { --_Result; }
...
_NODISCARD inline uint32_t _Bit_scan_reverse(const uint32_t _Value) noexcept {
  ...
  if (_BitScanReverse(&_Index, _Value)) { return _Index + 1; }
...
#ifdef _WIN64
  if (_BitScanReverse64(&_Index, _Value)) { return _Index + 1; }
#else  // ^^^ 64-bit / 32-bit vvv
  uint32_t _Ui32 = static_cast<uint32_t>(_Value >> 32);

  if (_BitScanReverse(&_Index, _Ui32)) { return _Index + 1 + 32; }

  _Ui32 = static_cast<uint32_t>(_Value);

  if (_BitScanReverse(&_Index, _Ui32)) { return _Index + 1; }
#endif // ^^^ 32-bit ^^^
