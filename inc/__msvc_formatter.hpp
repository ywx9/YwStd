...
#if _HAS_CXX23
_EXPORT_STD template<class _Ty> constexpr bool enable_nonlocking_formatter_optimization = false;
...
template<class _CharT> struct _Basic_format_specs {
  int _Width = 0;
  int _Precision = -1;
  char _Type = '\0';
...
template<class _CharT> struct _Dynamic_format_specs : _Basic_format_specs<_CharT> {
...
template<class _CharT> concept _Format_supported_charT = _Is_any_of_v<_CharT, char, wchar_t>;

_EXPORT_STD template<class _Ty, class _CharT = char> struct formatter {
  formatter() = delete;
...
template<class _Ty, class _CharT, class _FormatContext> _FormatContext::iterator
_Formatter_base_format(const _Dynamic_format_specs<_CharT>& _Specs, const _Ty& _Val, _FormatContext& _FormatCtx);

template<class _Ty, class _CharT, _Basic_format_arg_type _ArgType> struct _Formatter_base {
public:
#if _HAS_CXX23
  constexpr void _Set_debug_format() noexcept requires (_Is_debug_enabled_fmt_type(_ArgType)) { _Specs._Type = '?'; }
#endif // _HAS_CXX23
  ...
  template<class _FormatContext> _FormatContext::iterator format(const _Ty& _Val, _FormatContext& _FormatCtx) const {
...
#define _FORMAT_SPECIALIZE_NONLOCKING_FOR(_Type)                                           \
  template<> inline constexpr bool enable_nonlocking_formatter_optimization<_Type> = true;
#else // ^^^ _HAS_CXX23 / !_HAS_CXX23 vvv
#define _FORMAT_SPECIALIZE_NONLOCKING_FOR(_Type)
#endif // ^^^ !_HAS_CXX23 ^^^
...
#define _FORMAT_SPECIALIZE_FOR(_Type, _ArgType) \
  ...
  template<_Format_supported_charT _CharT> struct formatter<_Type, _CharT> : _Formatter_base<_Type, _CharT, _ArgType> {}
...
template<_Format_supported_charT _CharT> struct formatter<char, _CharT>
  : _Formatter_base<char, _CharT, _Basic_format_arg_type::_Char_type> {
#if _HAS_CXX23
  constexpr void set_debug_format() noexcept { this->_Set_debug_format(); }
#endif // _HAS_CXX23
};

// not using the macro because we'd like to avoid the formatter<wchar_t, char> specialization
template<> struct formatter<wchar_t, wchar_t> : _Formatter_base<wchar_t, wchar_t, _Basic_format_arg_type::_Char_type> {
#if _HAS_CXX23
  constexpr void set_debug_format() noexcept { _Set_debug_format(); }
#endif // _HAS_CXX23
};
...
template<_Format_supported_charT _CharT> struct formatter<_CharT*, _CharT>
  : _Formatter_base<_CharT*, _CharT, _Basic_format_arg_type::_CString_type> {
#if _HAS_CXX23
  constexpr void set_debug_format() noexcept { this->_Set_debug_format(); }
#endif // _HAS_CXX23
};

template<_Format_supported_charT _CharT> struct formatter<const _CharT*, _CharT>
...
template<_Format_supported_charT _CharT, size_t _Nx> struct formatter<_CharT[_Nx], _CharT>
...
_EXPORT_STD template<class _Elem, class _Traits, class _Alloc> class basic_string;

_EXPORT_STD template<class _Elem, class _Traits> class basic_string_view;
...
template<_Format_supported_charT _CharT, class _Traits> struct formatter<basic_string_view<_CharT, _Traits>, _CharT>
...
template<size_t _Size> struct formatter<char[_Size], wchar_t> {
  formatter() = delete;
  formatter(const formatter&) = delete;
  formatter& operator=(const formatter&) = delete;
};

template<class _Traits, class _Allocator> struct formatter<basic_string<char, _Traits, _Allocator>, wchar_t> {
  formatter() = delete;
  formatter(const formatter&) = delete;
  formatter& operator=(const formatter&) = delete;
};

template<class _Traits> struct formatter<basic_string_view<char, _Traits>, wchar_t> {
  formatter() = delete;
  formatter(const formatter&) = delete;
  formatter& operator=(const formatter&) = delete;
};
...
template<class _Ty> struct _Invalid_format_kind {
...
_EXPORT_STD template<class _Ty> constexpr _Invalid_format_kind<_Ty> format_kind;

template<class _Ty> constexpr bool _Is_two_tuple = false;

template<class _Ty, class _Uty> constexpr bool _Is_two_tuple<pair<_Ty, _Uty>> = true;

template<class _Ty, class _Uty> constexpr bool _Is_two_tuple<tuple<_Ty, _Uty>> = true;

template<_RANGES input_range _Rng> requires same_as<_Rng, remove_cvref_t<_Rng>>
...
template<class _Rng> concept _Formatting_enabled_range = format_kind<_Rng> != range_format::disabled;
...
template<_Format_supported_charT _CharT> constexpr bool enable_nonlocking_formatter_optimization<_CharT> = true;

template<_Format_supported_charT _CharT> constexpr bool enable_nonlocking_formatter_optimization<_CharT*> = true;

template<_Format_supported_charT _CharT> constexpr bool enable_nonlocking_formatter_optimization<const _CharT*> = true;
...
template<class _Ty1, class _Ty2> constexpr bool enable_nonlocking_formatter_optimization<pair<_Ty1, _Ty2>> =
  enable_nonlocking_formatter_optimization<_Ty1> && enable_nonlocking_formatter_optimization<_Ty2>;

template<class... _Ts> constexpr bool enable_nonlocking_formatter_optimization<tuple<_Ts...>> =
  (enable_nonlocking_formatter_optimization<_Ts> && ...);

template<class _CharT> struct _Fill_align_and_width_specs {
  int _Width = -1;
  int _Dynamic_width_index = -1;
  _Fmt_align _Alignment = _Fmt_align::_None;
  uint8_t _Fill_length = 1;
...
// TRANSITION, VSO-1236041: Avoid declaring and defining member functions in different headers.
template<class _CharT, class _Pc> _NODISCARD constexpr _Pc::iterator
_Fill_align_and_width_formatter_parse(_Fill_align_and_width_specs<_CharT>& _Specs, _Pc& _Parse_ctx);

template<class _CharT, class _FormatContext, class _Func> _NODISCARD _FormatContext::iterator
_Fill_align_and_width_formatter_format(const _Fill_align_and_width_specs<_CharT>& _Specs, _FormatContext& _Format_ctx,
                                       int _Width, _Fmt_align _Default_align, _Func&& _Fn);

template<class _CharT> struct _Fill_align_and_width_formatter {
public:
  ...
  template<class _FormatContext, class _Func> _NODISCARD constexpr auto
  _Format(_FormatContext& _Format_ctx, const int _Width, _Fmt_align _Default_align, _Func&& _Fn) const {
    return _STD _Fill_align_and_width_formatter_format(_Specs, _Format_ctx, _Width, _Default_align,
                                                       _STD forward<_Func>(_Fn));
  }
