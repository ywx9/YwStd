...
template<class _Elem> bool _Fgetc(_Elem& _Ch, FILE* _File) { // get an element from a C stream
  return _CSTD fread(&_Ch, sizeof(_Elem), 1, _File) == 1;
}

template<> inline bool _Fgetc(char& _Byte, FILE* _File) { // get a char element from a C stream
  int _Meta;
...
template<> inline bool _Fgetc(wchar_t& _Wchar, FILE* _File) { // get a wchar_t element from a C stream
  wint_t _Meta;
...
#ifdef _CRTBLD
template<> inline bool _Fgetc(unsigned short& _Wchar, FILE* _File) { // get an unsigned short element from a C stream
  wint_t _Meta;
  if ((_Meta = _CSTD fgetwc(_File)) == WEOF) {
    return false;
...
template<class _Elem> bool _Fputc(_Elem _Ch, FILE* _File) { // put an element to a C stream
  return _CSTD fwrite(&_Ch, 1, sizeof(_Elem), _File) == sizeof(_Elem);
}

template<> inline bool _Fputc(char _Byte, FILE* _File) { // put a char element to a C stream
  return _CSTD fputc(_Byte, _File) != EOF;
}

template<> inline bool _Fputc(wchar_t _Wchar, FILE* _File) { // put a wchar_t element to a C stream
  return _CSTD fputwc(_Wchar, _File) != WEOF;
}

#ifdef _CRTBLD
template<> inline bool _Fputc(unsigned short _Wchar, FILE* _File) { // put an unsigned short element to a C stream
  return _CSTD fputwc(_Wchar, _File) != WEOF;
}
#endif // defined(_CRTBLD)

template<class _Elem> bool _Ungetc(const _Elem&, FILE*) { // put back an arbitrary element to a C stream (always fail)
  return false;
}

template<> inline bool _Ungetc(const char& _Byte, FILE* _File) { // put back a char element to a C stream
  return _CSTD ungetc(static_cast<unsigned char>(_Byte), _File) != EOF;
}

template<> inline bool _Ungetc(const signed char& _Byte, FILE* _File) { // put back a signed char element to a C stream
  return _CSTD ungetc(static_cast<unsigned char>(_Byte), _File) != EOF;
}
...
template<> inline bool _Ungetc(const wchar_t& _Wchar, FILE* _File) { // put back a wchar_t element to a C stream
...
_EXPORT_STD template<class _Elem, class _Traits> class basic_filebuf
  : public basic_streambuf<_Elem, _Traits> { // stream buffer associated with a C stream
public:
  using _Mysb = basic_streambuf<_Elem, _Traits>;
  using _Cvt = codecvt<_Elem, char, typename _Traits::state_type>;

  basic_filebuf() : _Mysb() { _Init(nullptr, _Newfl); }
  ...
  _NODISCARD bool is_open() const noexcept /* strengthened */ { return static_cast<bool>(_Myfile); }
  ...
  basic_filebuf* close() {
    ...
    if (_Myfile) {   // put any homing sequence and close file
      ...
      _Ans = this;
      if (!_Endwrite()) { _Ans = nullptr; }

      if (_CSTD fclose(_Myfile) != 0) { _Ans = nullptr; }
    } else {
      _Ans = nullptr;
    }
...
protected:
  int_type __CLR_OR_THIS_CALL overflow(int_type _Meta = _Traits::eof()) override { // put an element to stream
    ...
    default: return _Traits::eof(); // conversion failed
    }
  }
  ...
  int_type __CLR_OR_THIS_CALL pbackfail(int_type _Meta = _Traits::eof()) override {
    ...
    if (_Mysb::gptr() && _Mysb::eback() < _Mysb::gptr() &&
        (_Traits::eq_int_type(_Traits::eof(), _Meta) || _Traits::eq_int_type(_Traits::to_int_type(_Mysb::gptr()[-1]),
    ...
    for (;;) { // get using codecvt facet
      ...
      case codecvt_base::ok:
        if (_Dest != &_Ch) { // got an element, put back excess and deliver it
          ...
          while (0 < _Nleft) { _CSTD ungetc(_Src[--_Nleft], _Myfile); }
    ...
    if constexpr (sizeof(_Elem) == 1) {
      ...
      if (_Myfile) {   // open C stream, attempt read
        ...
        while (_Read_size < _Count_s) {
          ...
          if (_Actual_read != _Read_size) { return static_cast<streamsize>(_Start_count - _Count_s); }
        }

        if (0 < _Count_s) { _Count_s -= _CSTD fread(_Ptr, sizeof(_Elem), _Count_s, _Myfile); }
      }
  ...
  pos_type __CLR_OR_THIS_CALL seekoff(off_type _Off, ios_base::seekdir _Way,
                                      ios_base::openmode = ios_base::in |
                                                           ios_base::out) override { // change position by _Off
  ...
  int __CLR_OR_THIS_CALL sync() override { // synchronize C stream with external file
    if (!_Myfile || _Traits::eq_int_type(_Traits::eof(), overflow()) || 0 <= _CSTD fflush(_Myfile)) { return 0; }
  ...
  void _Init(FILE* _File, _Initfl _Which) noexcept { // initialize to C stream _File after {new, open, close}
    ...
    if (_File && sizeof(_Elem) == 1) { // point inside C stream with [first, first + count) buffer
      ...
      ::_get_stream_buffer_pointers(_File, reinterpret_cast<char***>(&_Pb), reinterpret_cast<char***>(&_Pn), &_Nr);
      int* _Nw = _Nr;
  ...
  bool _Endwrite() { // put shift to initial conversion state, as needed
    if (!_Pcvt || !_Wrotesome) { return true; }
    ...
    case codecvt_base::partial: { // put any generated bytes
      const auto _Count = static_cast<size_t>(_Dest - _Str);
      if (0 < _Count && _Count != static_cast<size_t>(_CSTD fwrite(_Str, 1, _Count, _Myfile))) {
    ...
    default: return false; // conversion failed
    }
  }
...
private:
  ...
  void _Reset_back() noexcept { // restore buffer after putback
    if (_Mysb::eback() == &_Mychar) { _Mysb::setg(_Set_eback, _Set_eback, _Set_egptr); }
  }
