...
void _Uses_alloc_construct_pair_piecewise(_CvPair* const _Ptr, _Outer_alloc& _Outer, _Inner_alloc& _Inner,
  ...
  allocator_traits<_Outer_alloc>::construct(
    _Outer, _Ptr, piecewise_construct,
...
void _Uses_alloc_construct_pair(_CvPair* const _Ptr, _Outer_alloc& _Outer, _Inner_alloc& _Inner, _Uty&& _Arg1,
                                _Vty&& _Arg2) {
...
void _Uses_alloc_construct_pair(_CvPair* const _Ptr, _Outer_alloc& _Outer, _Inner_alloc& _Inner,
  ...
  _STD _Uses_alloc_construct_pair_piecewise(_Ptr, _Outer, _Inner, _STD forward_as_tuple(_Pair.first),
                                            _STD forward_as_tuple(_Pair.second));
}

template<class _CvPair, class _Outer_alloc, class _Inner_alloc, class _Uty, class _Vty> void
_Uses_alloc_construct_pair(_CvPair* const _Ptr, _Outer_alloc& _Outer, _Inner_alloc& _Inner, pair<_Uty, _Vty>&& _Pair) {
...
void _Uses_alloc_construct_pair(_CvPair* const _Ptr, _Outer_alloc& _Outer, _Inner_alloc& _Inner, _Uty&& _Ux) {
  ...
  if constexpr (is_lvalue_reference_v<_Pair_ref_t>) {
    _STD _Uses_alloc_construct_pair_piecewise(_Ptr, _Outer, _Inner, _STD forward_as_tuple(_Pair_ref.first),
                                              _STD forward_as_tuple(_Pair_ref.second));
  } else {
    _STD _Uses_alloc_construct_pair_piecewise(
      _Ptr, _Outer, _Inner, _STD forward_as_tuple(_STD forward<typename _CvPair::first_type>(_Pair_ref.first)),
...
public:
  ...
  _NODISCARD_RAW_PTR_ALLOC __declspec(allocator) void*
  allocate(_CRT_GUARDOVERFLOW const size_t _Bytes,
...
_EXPORT_STD _NODISCARD inline bool operator==(const memory_resource& _Left, const memory_resource& _Right) noexcept {
...
#if _HAS_CXX20
template<class _Uty> struct _NODISCARD _Deallocate_bytes_guard {
  ~_Deallocate_bytes_guard() noexcept {
    if (_Mem_res) { _Mem_res->deallocate(_Pobj, sizeof(_Uty), alignof(_Uty)); }
  }
...
public:
  template<class> friend class polymorphic_allocator;
  ...
  template<class _Uty> polymorphic_allocator(const polymorphic_allocator<_Uty>& _That) noexcept
...
#if _HAS_CXX20
  _NODISCARD_RAW_PTR_ALLOC __declspec(allocator) void* allocate_bytes(const size_t _Bytes,
                                                                      const size_t _Align = alignof(max_align_t)) {
    return _Resource->allocate(_Bytes, _Align);
  }

  void deallocate_bytes(void* const _Ptr, const size_t _Bytes, const size_t _Align = alignof(max_align_t)) noexcept
  /* strengthened */ {
    _Resource->deallocate(_Ptr, _Bytes, _Align);
  }

  template<class _Uty>
  _NODISCARD_RAW_PTR_ALLOC __declspec(allocator) _Uty* allocate_object(_CRT_GUARDOVERFLOW const size_t _Count = 1) {
    void* const _Vp = allocate_bytes(_Get_size_of_n<sizeof(_Uty)>(_Count), alignof(_Uty));
    return static_cast<_Uty*>(_Vp);
  }

  template<class _Uty> void deallocate_object(_Uty* const _Ptr, const size_t _Count = 1) noexcept /* strengthened */ {
  ...
  template<class _Uty> void delete_object(_Uty* const _Ptr) noexcept /* strengthened */ {
    _Ptr->~_Uty();
    deallocate_object(_Ptr);
  }
#endif // _HAS_CXX20

  template<class _Uty, class... _Types> void construct(_Uty* const _Ptr, _Types&&... _Args) {
  ...
  template<class _Uty> void destroy(_Uty* const _Ptr) noexcept /* strengthened */ { _Ptr->~_Uty(); }
  ...
  _NODISCARD friend bool operator==(const polymorphic_allocator& _Lhs, const polymorphic_allocator& _Rhs) noexcept {
...
_EXPORT_STD template<class _Ty1, class _Ty2> _NODISCARD bool
operator==(const polymorphic_allocator<_Ty1>& _Left, const polymorphic_allocator<_Ty2>& _Right) noexcept {
  // polymorphic_allocators with the same resource are compatible
  return *_Left.resource() == *_Right.resource();
}

#if !_HAS_CXX20
template<class _Ty1, class _Ty2> _NODISCARD bool operator!=(const polymorphic_allocator<_Ty1>& _Left,
...
template<class _Ty, class _Ptr> struct _Has_no_alloc_destroy<pmr::polymorphic_allocator<_Ty>, _Ptr, void> : true_type {
