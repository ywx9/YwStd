...
public:
  _CancellationTokenRegistration(long _InitialRefs = 1)
    : _RefCounter(_InitialRefs), _M_state(_STATE_CALLED), _M_signaled(false), _M_pTokenState(nullptr) {}
...
public:
  ...
  void _DeregisterCallback(_In_ _CancellationTokenRegistration* _PRegistration) {
...

    {
      ...
      //
      // If a cancellation has occurred, the registration list is guaranteed to be empty if we've observed it under the
      // auspices of the lock.  In this case, we must synchronize with the cancelling thread to guarantee that the
      // cancellation is finished by the time we return from this method.
      //
    ...
    if (_Synchronize) {
      ...
      default: {
        if (_Result == ::Concurrency::details::platform::GetCurrentThreadId()) {
          //
          // It is entirely legal for a caller to Deregister during a callback instead of having to provide their own
          // synchronization mechanism between the two.  In this case, we do *NOT* need to explicitly synchronize with
          // the callback as doing so would deadlock.  If the call happens during, skip any extra synchronization.
          //
          break;
        }
...
public:
  ...
  ///     Note that if the token is already canceled at the point where this method is called, the callback will be made
...
public:
  ...
  /// </summary>
  /// <param name="_Src">
  ///     A token whose cancellation will cause cancellation of the returned token source.  Note that the returned token
  ///     source can also be canceled independently of the source contained in this parameter.
  /// </param>
  /// <returns>
  ...
  /// <summary>
  ///     Creates a <c>cancellation_token_source</c> which is canceled when one of a series of tokens represented by an
  ///     STL iterator pair is canceled.
  /// </summary>
  /// <param name="_Begin">
  ...
  /// </param>
  /// <returns>
  ///     A <c>cancellation_token_source</c> which is canceled when any of the tokens provided by the range described by
  ///     the STL iterators contained in the <paramref name="_Begin"/> and <paramref name="_End"/> parameters is
  ///     canceled.
  /// </returns>
