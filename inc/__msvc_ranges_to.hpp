...
_STD_BEGIN
namespace ranges {
template<class _Ty> constexpr bool _Is_initializer_list = _Is_specialization_v<remove_cvref_t<_Ty>, initializer_list>;

_EXPORT_STD template<class _Rng> concept viewable_range =
  range<_Rng> && ((view<remove_cvref_t<_Rng>> && constructible_from<remove_cvref_t<_Rng>, _Rng>) ||
                  (!view<remove_cvref_t<_Rng>> && (is_lvalue_reference_v<_Rng> || (movable<remove_reference_t<_Rng>> &&
                                                                                   !_Is_initializer_list<_Rng>))));

namespace _Pipe {
template<class _Derived> struct _Base {};

template<class _Ty> _Ty* _Derived_from_range_adaptor_closure(_Base<_Ty>&); // not defined

template<class _Ty> concept _Range_adaptor_closure_object =
  !range<remove_cvref_t<_Ty>> && requires(remove_cvref_t<_Ty>& __t) {
    { _Pipe::_Derived_from_range_adaptor_closure(__t) } -> same_as<remove_cvref_t<_Ty>*>;
  };

template<class _ClosureLeft, class _ClosureRight> struct _Pipeline : _Base<_Pipeline<_ClosureLeft, _ClosureRight>> {
  ...
  constexpr explicit _Pipeline(_Ty1&& _Val1, _Ty2&& _Val2) noexcept(is_nothrow_constructible_v<_Ty1, _ClosureLeft> &&
                                                                    is_nothrow_constructible_v<_Ty2, _ClosureRight>)
  ...
  template<class _Ty> _NODISCARD constexpr decltype(auto)
  operator()(_Ty&& _Val) & noexcept(noexcept(_Right(_Left(_STD forward<_Ty>(_Val)))))
    requires requires { _Right(_Left(_STD forward<_Ty>(_Val))); } {
    return _Right(_Left(_STD forward<_Ty>(_Val)));
  }

  template<class _Ty> _NODISCARD constexpr decltype(auto)
  operator()(_Ty&& _Val) const& noexcept(noexcept(_Right(_Left(_STD forward<_Ty>(_Val)))))
    requires requires { _Right(_Left(_STD forward<_Ty>(_Val))); } {
    return _Right(_Left(_STD forward<_Ty>(_Val)));
  }

  template<class _Ty> _NODISCARD constexpr decltype(auto)
  operator()(_Ty&& _Val) && noexcept(noexcept(_STD move(_Right)(_STD move(_Left)(_STD forward<_Ty>(_Val)))))
  ...
  template<class _Ty> _NODISCARD constexpr decltype(auto)
  operator()(_Ty&& _Val) const&& noexcept(noexcept(_STD move(_Right)(_STD move(_Left)(_STD forward<_Ty>(_Val)))))
    requires requires { _STD move(_Right)(_STD move(_Left)(_STD forward<_Ty>(_Val))); } {
    return _STD move(_Right)(_STD move(_Left)(_STD forward<_Ty>(_Val)));
  }
};
...
requires _Range_adaptor_closure_object<_Left> && _Range_adaptor_closure_object<_Right> &&
         constructible_from<remove_cvref_t<_Left>, _Left> && constructible_from<remove_cvref_t<_Right>, _Right>
_NODISCARD constexpr auto operator|(_Left&& __l, _Right&& __r) noexcept(noexcept(_Pipeline{
  static_cast<_Left&&>(__l), static_cast<_Right&&>(__r)})) {
  return _Pipeline{static_cast<_Left&&>(__l), static_cast<_Right&&>(__r)};
}

_EXPORT_STD template<class _Left, class _Right> requires (_Range_adaptor_closure_object<_Right> && range<_Left>)
_NODISCARD constexpr decltype(auto)
operator|(_Left&& __l, _Right&& __r) noexcept(noexcept(_STD forward<_Right>(__r)(_STD forward<_Left>(__l))))
  requires requires { static_cast<_Right&&>(__r)(static_cast<_Left&&>(__l)); } {
  return _STD forward<_Right>(__r)(_STD forward<_Left>(__l));
}
} // namespace _Pipe

template<class _Ty> concept _Valid_movable_box_object =
#if _HAS_CXX23
...
template<_Valid_movable_box_object _Ty> class _Movable_box {
public:
  constexpr _Movable_box() noexcept(is_nothrow_default_constructible_v<_Ty>) requires default_initializable<_Ty>
    : _Val(), _Engaged{true} {}

  template<class... _Types>
  constexpr _Movable_box(in_place_t,
                         _Types&&... _Args) noexcept(is_nothrow_constructible_v<_Ty, _Types...>) // strengthened
    : _Val(_STD forward<_Types>(_Args)...), _Engaged{true} {}

  constexpr ~_Movable_box() {
    if (_Engaged) { _Val.~_Ty(); }
  }

  ~_Movable_box() requires is_trivially_destructible_v<_Ty> = default;

  _Movable_box(const _Movable_box&) requires copy_constructible<_Ty> && is_trivially_copy_constructible_v<_Ty>
  = default;

  constexpr _Movable_box(const _Movable_box& _That) requires copy_constructible<_Ty> : _Engaged{_That._Engaged} {
    if (_That._Engaged) { _STD _Construct_in_place(_Val, static_cast<const _Ty&>(_That._Val)); }
  }
  ...
  constexpr _Movable_box(_Movable_box&& _That) : _Engaged{_That._Engaged} {
    if (_That._Engaged) { _STD _Construct_in_place(_Val, static_cast<_Ty&&>(_That._Val)); }
  }

  _Movable_box& operator=(const _Movable_box&) noexcept requires copyable<_Ty> && is_trivially_copy_assignable_v<_Ty>
  = default;

  constexpr _Movable_box&
  operator=(const _Movable_box& _That) noexcept(is_nothrow_copy_constructible_v<_Ty> &&
                                                is_nothrow_copy_assignable_v<_Ty>) // strengthened
  ...
  constexpr _Movable_box& operator=(const _Movable_box& _That) noexcept(is_nothrow_copy_constructible_v<_Ty>)
    ...
    if (_STD addressof(_That) == this) { return *this; }
  ...
  _Movable_box& operator=(_Movable_box&&) noexcept requires movable<_Ty> && is_trivially_move_assignable_v<_Ty>
  = default;
...
// [range.move.wrap]
template<class _Ty> concept _Use_simple_movable_box_wrapper =
  (copy_constructible<_Ty>
...
template<class _Ty> concept _Copy_constructible_for_box = is_copy_constructible_v<_Ty>;

template<_Valid_movable_box_object _Ty> requires _Use_simple_movable_box_wrapper<_Ty>
...
public:
  ...
  _Movable_box(const _Movable_box&) requires _Copy_constructible_for_box<_Ty> && is_trivially_copy_constructible_v<_Ty>
  = default;
  _Movable_box(_Movable_box&&) requires is_trivially_move_constructible_v<_Ty> = default;
  ...
  _Movable_box& operator=(const _Movable_box&) requires copyable<_Ty> && is_trivially_copy_assignable_v<_Ty> = default;
  _Movable_box& operator=(_Movable_box&&) requires movable<_Ty> && is_trivially_move_assignable_v<_Ty> = default;

  constexpr _Movable_box& operator=(const _Movable_box& _That) noexcept(is_nothrow_copy_assignable_v<_Ty> ||
                                                                        !copyable<_Ty>) // strengthened
    requires copy_constructible<_Ty> {
  ...
  constexpr _Movable_box& operator=(_Movable_box&& _That) noexcept(is_nothrow_move_assignable_v<_Ty> ||
                                                                   !movable<_Ty>) /* strengthened */ {
  ...
  _NODISCARD constexpr _Ty& operator*() noexcept { return _Val; }
  _NODISCARD constexpr const _Ty& operator*() const noexcept { return _Val; }

private:
  /* [[no_unique_address]] */ remove_cv_t<_Ty> _Val{};
};

template<class _Fn, class... _Types> class _Range_closure : public _Pipe::_Base<_Range_closure<_Fn, _Types...>> {
public:
  ...
  template<class... _UTypes> requires (same_as<decay_t<_UTypes>, _Types> && ...) constexpr explicit _Range_closure(
    _UTypes&&... _Args) noexcept(conjunction_v<is_nothrow_constructible<_Types, _UTypes>...>)
  ...
  template<class _Ty> requires invocable<_Fn, _Ty, _Types&...> constexpr decltype(auto)
  operator()(_Ty&& _Arg) & noexcept(noexcept(_Call(*this, _STD forward<_Ty>(_Arg), _Indices{}))) {
    return _Call(*this, _STD forward<_Ty>(_Arg), _Indices{});
  }

  template<class _Ty> requires invocable<_Fn, _Ty, const _Types&...> constexpr decltype(auto)
  operator()(_Ty&& _Arg) const& noexcept(noexcept(_Call(*this, _STD forward<_Ty>(_Arg), _Indices{}))) {
    return _Call(*this, _STD forward<_Ty>(_Arg), _Indices{});
  }

  template<class _Ty> requires invocable<_Fn, _Ty, _Types...> constexpr decltype(auto)
  operator()(_Ty&& _Arg) && noexcept(noexcept(_Call(_STD move(*this), _STD forward<_Ty>(_Arg), _Indices{}))) {
    return _Call(_STD move(*this), _STD forward<_Ty>(_Arg), _Indices{});
  }

  template<class _Ty> requires invocable<_Fn, _Ty, const _Types...> constexpr decltype(auto)
  operator()(_Ty&& _Arg) const&& noexcept(noexcept(_Call(_STD move(*this), _STD forward<_Ty>(_Arg), _Indices{}))) {
    return _Call(_STD move(*this), _STD forward<_Ty>(_Arg), _Indices{});
  }
...
_EXPORT_STD template<range _Rng> requires is_object_v<_Rng> class ref_view : public view_interface<ref_view<_Rng>> {
private:
  _Rng* _Range;
...
public:
  template<_Different_from<ref_view> _OtherRng> constexpr ref_view(_OtherRng&& _Other) noexcept(
  ...
  _NODISCARD constexpr _Rng& base() const noexcept /* strengthened */ { return *_Range; }

  _NODISCARD constexpr iterator_t<_Rng> begin() const noexcept(noexcept(_RANGES begin(*_Range))) /* strengthened */ {
...
template<class _Rng> ref_view(_Rng&) -> ref_view<_Rng>;

template<class _Rng> constexpr bool enable_borrowed_range<ref_view<_Rng>> = true;

_EXPORT_STD template<range _Rng> requires (movable<_Rng> && !_Is_initializer_list<_Rng>)
...
public:
  ...
  _NODISCARD constexpr _Rng& base() & noexcept { return _Range; }
  _NODISCARD constexpr const _Rng& base() const& noexcept { return _Range; }
  _NODISCARD constexpr _Rng&& base() && noexcept { return _STD move(_Range); }
  _NODISCARD constexpr const _Rng&& base() const&& noexcept { return _STD move(_Range); }
...
template<class _Rng> constexpr bool enable_borrowed_range<owning_view<_Rng>> = enable_borrowed_range<_Rng>;

namespace views {
template<class _Rng> concept _Can_ref_view = requires(_Rng&& __r) { ref_view{static_cast<_Rng&&>(__r)}; };

template<class _Rng> concept _Ownable = requires(_Rng&& __r) { owning_view{static_cast<_Rng&&>(__r)}; };

class _All_fn : public _Pipe::_Base<_All_fn> {
private:
  enum class _St { _None, _View, _Ref, _Own };

  template<class _Rng> _NODISCARD static consteval _Choice_t<_St> _Choose() noexcept {
  ...
  template<class _Rng> static constexpr _Choice_t<_St> _Choice = _Choose<_Rng>();

public:
  template<viewable_range _Rng> requires (_Choice<_Rng>._Strategy != _St::_None)
...
_EXPORT_STD template<viewable_range _Rng> using all_t = decltype(all(_STD declval<_Rng>()));
} // namespace views
...
private:
  ...
  template<bool _Const> struct _Category_base {};

  template<bool _Const> requires forward_range<_Maybe_const<_Const, _Vw>> struct _Category_base<_Const> {
  ...
  template<bool _Const> class _Iterator : public _Category_base<_Const> {
  private:
  ...
  public:
    ...
    constexpr _Iterator(_Parent_t& _Parent_, iterator_t<_Base> _Current_) noexcept(
      ...
      if constexpr (forward_range<_Base>) { _STD _Adl_verify_range(_RANGES begin(_Parent_._Range), _Current); }
#endif // _ITERATOR_DEBUG_LEVEL != 0
    }
    ...
    _NODISCARD constexpr const iterator_t<_Base>& base() const& noexcept { return _Current; }
    _NODISCARD constexpr iterator_t<_Base>
    base() && noexcept(is_nothrow_move_constructible_v<iterator_t<_Base>>) /* strengthened */ {
      return _STD move(_Current);
    }

    _NODISCARD constexpr decltype(auto) operator*() const noexcept(noexcept(_STD invoke(*_Parent->_Fun, *_Current))) {
#if _ITERATOR_DEBUG_LEVEL != 0
      _Check_dereference();
      _STL_VERIFY(_Parent->_Fun, "Cannot dereference iterator into transform_view with no transformation function");
#endif // _ITERATOR_DEBUG_LEVEL != 0
    ...
    constexpr _Iterator& operator++() noexcept(noexcept(++_Current)) /* strengthened */ {
      ...
      _STL_VERIFY(_Current != _RANGES end(_Parent->_Range), "Cannot increment transform_view iterator past end");
#endif // _ITERATOR_DEBUG_LEVEL != 0
      ++_Current;
      return *this;
    }
    ...
    constexpr _Iterator operator--(int) noexcept(noexcept(--_Current) &&
...
#if _ITERATOR_DEBUG_LEVEL != 0
    constexpr void _Verify_offset(const difference_type _Off) const noexcept requires random_access_range<_Base> {
      _STL_VERIFY(_Off == 0 || _Parent, "cannot seek value-initialized transform_view iterator");
      if constexpr (_Offset_verifiable_v<iterator_t<_Base>>) { _Current._Verify_offset(_Off); }
    }
#endif // _ITERATOR_DEBUG_LEVEL != 0

    constexpr _Iterator& operator+=(const difference_type _Off) noexcept(noexcept(_Current += _Off)) /* strengthened */
    ...
    operator==(const _Iterator& _Left,
    ...
    _NODISCARD friend constexpr _Iterator
    operator+(_Iterator _It, const difference_type _Off) noexcept(noexcept(_It._Current += _Off)) /* strengthened */
      requires random_access_range<_Base> {
#if _ITERATOR_DEBUG_LEVEL != 0
    ...
    _NODISCARD friend constexpr _Iterator
    operator+(const difference_type _Off, _Iterator _It) noexcept(noexcept(_It._Current += _Off)) /* strengthened */
      requires random_access_range<_Base> {
#if _ITERATOR_DEBUG_LEVEL != 0
      _It._Verify_offset(_Off);
#endif // _ITERATOR_DEBUG_LEVEL != 0
  ...
  private:
    ...
    using _Parent_t = _Maybe_const<_Const, transform_view>;
    using _Base = _Maybe_const<_Const, _Vw>;
    template<bool _OtherConst> using _Maybe_const_iter = iterator_t<_Maybe_const<_OtherConst, _Vw>>;

    sentinel_t<_Base> _Last{};

    template<bool _OtherConst> _NODISCARD static constexpr const _Maybe_const_iter<_OtherConst>&
    _Get_current(const _Iterator<_OtherConst>& _It) noexcept {
#if _ITERATOR_DEBUG_LEVEL != 0
      _STL_VERIFY(_It._Parent != nullptr, "cannot compare transform_view sentinel with value-initialized iterator");
#endif // _ITERATOR_DEBUG_LEVEL != 0
      return _It._Current;
    }
  ...
  public:
    ...
    template<bool _OtherConst> requires sentinel_for<sentinel_t<_Base>, _Maybe_const_iter<_OtherConst>>
    _NODISCARD friend constexpr bool
    operator==(const _Iterator<_OtherConst>& _Left,
    ...
    template<bool _OtherConst> requires sized_sentinel_for<sentinel_t<_Base>, _Maybe_const_iter<_OtherConst>>
    ...
    _NODISCARD friend constexpr range_difference_t<_Maybe_const<_OtherConst, _Vw>> operator-(
      const _Sentinel& _Left,
      const _Iterator<_OtherConst>& _Right) noexcept(noexcept(_Left._Last - _Get_current(_Right))) /* strengthened */ {
...
public:
  ...
  constexpr explicit transform_view(_Vw _Range_,
                                    _Fn _Fun_) noexcept(is_nothrow_move_constructible_v<_Vw> &&
                                                        is_nothrow_move_constructible_v<_Fn>) // strengthened
  ...
  _NODISCARD constexpr _Iterator<false>
  begin() noexcept(noexcept(_RANGES begin(_Range)) &&
  ...
  _NODISCARD constexpr _Iterator<true> begin() const
    noexcept(noexcept(_RANGES begin(_Range)) && is_nothrow_move_constructible_v<iterator_t<_Vw>>) /* strengthened */
  ...
  end() noexcept(noexcept(_RANGES end(_Range)) &&
...
template<class _Rng, class _Fn> transform_view(_Rng&&, _Fn) -> transform_view<views::all_t<_Rng>, _Fn>;
...
struct _Transform_fn {
  ...
  template<class _Fn> requires constructible_from<decay_t<_Fn>, _Fn>
...
#if _HAS_CXX23
template<class _Range, class _Container> concept _Sized_and_reservable =
  sized_range<_Range> && sized_range<_Container> && requires(_Container& _Cont, const range_size_t<_Container> _Count) {
    _Cont.reserve(_Count);
    { _Cont.capacity() } -> same_as<range_size_t<_Container>>;
    { _Cont.max_size() } -> same_as<range_size_t<_Container>>;
  };

template<class _Rng, class _Container> concept _Ref_converts =
  !input_range<_Container> || convertible_to<range_reference_t<_Rng>, range_value_t<_Container>>;

template<class _Rng, class _Container, class... _Types> concept _Common_constructible =
  common_range<_Rng> //
...
template<class _Container, class _Reference> concept _Can_emplace_back =
  requires(_Container& _Cont) { _Cont.emplace_back(_STD declval<_Reference>()); };

template<class _Container, class _Reference> concept _Can_push_back =
  requires(_Container& _Cont) { _Cont.push_back(_STD declval<_Reference>()); };

template<class _Container, class _Reference> concept _Can_emplace_end =
  requires(_Container& _Cont) { _Cont.emplace(_Cont.end(), _STD declval<_Reference>()); };

template<class _Container, class _Reference> concept _Can_insert_end =
  requires(_Container& _Cont) { _Cont.insert(_Cont.end(), _STD declval<_Reference>()); };

template<class _Rng, class _Container, class... _Types> concept _Constructible_appendable =
  constructible_from<_Container, _Types...> &&
  (_Can_emplace_back<_Container, range_reference_t<_Rng>> || _Can_push_back<_Container, range_reference_t<_Rng>> ||
   _Can_emplace_end<_Container, range_reference_t<_Rng>> || _Can_insert_end<_Container, range_reference_t<_Rng>>);

_EXPORT_STD template<class _Container, input_range _Rng, class... _Types> requires (!view<_Container>)
...
template<class _Container> struct _To_class_fn {
  ...
  _NODISCARD _STATIC_CALL_OPERATOR constexpr auto operator()(_Rng&& _Range, _Types&&... _Args) _CONST_CALL_OPERATOR
...
_EXPORT_STD template<class _Container, class... _Types> requires (!view<_Container>)
...
template<input_range _Rng> struct _Phony_input_iterator {
...
template<template<class...> class _Cnt, class _Rng, class... _Args> auto _To_helper() {
  if constexpr (requires { _Cnt(_STD declval<_Rng>(), _STD declval<_Args>()...); }) {
    ...
    return static_cast<decltype(_Cnt(from_range, _STD declval<_Rng>(), _STD declval<_Args>()...))*>(nullptr);
                         ...
                         _Cnt(_STD declval<_Phony_input_iterator<_Rng>>(), _STD declval<_Phony_input_iterator<_Rng>>(),
                              _STD declval<_Args>()...);
                       }) {
...
template<template<class...> class _Container> struct _To_template_fn {
  template<input_range _Rng, class... _Types,
           class _Deduced = remove_pointer_t<decltype(_To_helper<_Container, _Rng, _Types...>())>>
  _NODISCARD _STATIC_CALL_OPERATOR constexpr auto operator()(_Rng&& _Range, _Types&&... _Args) _CONST_CALL_OPERATOR {
    return _RANGES to<_Deduced>(_STD forward<_Rng>(_Range), _STD forward<_Types>(_Args)...);
